...
router.post('/sprint', [
  ...
  body('type').isIn(['start', 'end', 'review', 'retrospective']).withMessage('Invalid notification type'),
  ...
  try {
    ...
    const sprintData = {
      ...
      totalStories: 12,
      velocity: 42,
      reviewDate: new Date(Date.now() + 24 * 60 * 60 * 1000).toLocaleDateString()
    };
...
router.post('/story', [
  ...
  body('type').isIn(['assigned', 'completed', 'blocked']).withMessage('Invalid notification type'),
  body('recipients').isArray().withMessage('Recipients must be an array')
], async (req, res) => {
  try {
    const errors = validationResult(req);
    ...
    const storyData = {
      ...
      description: "Create a notification center with dropdown menu and real-time updates",
      storyPoints: 5,
      priority: 'high',
    ...
    const results = await Promise.all(recipients.map(async (recipient) => {
      try {
        if (process.env.NODE_ENV === 'development') {
          logger.info(`Story email would be sent to ${recipient}:`, emailContent);
          return { success: true, recipient };
        }
