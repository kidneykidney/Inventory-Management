...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      const { name, email, password, role, department, phone, address } =
        req.body;
      ...
      logger.info(
        `User created by admin: ${email} (created by: ${req.user.email})`
      );
...
router.put(
  ...
  async (req, res) => {
    try {
      ...
      const { id } = req.params;
      const { name, email, role, status, department, phone, address } =
        req.body;
      ...
      logger.info(
        `User updated: ${existingUser.email} (updated by: ${req.user.email})`
      );
...
router.put(
  ...
  async (req, res) => {
    try {
      ...
      logger.info(
        `User status updated: ${user.email} -> ${status} (by: ${req.user.email})`
      );
...
router.put(
  '/:id/role',
  [authenticateToken, requireAdmin, body('role').isIn(['admin', 'user'])],
  async (req, res) => {
    try {
      // Check validation errors
      const errors = validationResult(req);
