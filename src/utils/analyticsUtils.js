...
export const calculateBurndownAccuracy = (sprintData) => {
  if (!sprintData || sprintData.length === 0) return { accuracy: 0, accurateCount: 0 };
...
export const calculateTeamSatisfactionMetrics = (retrospectiveData) => {
  ...
  const metrics = retrospectiveData.reduce((acc, retro) => {
  ...
  const overallSatisfaction = (averageMorale + averageVelocityRating + averageQualityRating + averageCommunicationRating) / 4;
  
  // Calculate satisfaction trend
  let satisfactionTrend = 'stable';
  if (retrospectiveData.length >= 3) {
    const recentScores = retrospectiveData.slice(-3).map(retro => 
      (retro.teamMorale + retro.velocityRating + retro.qualityRating + retro.communicationRating) / 4
    );
...
export const calculateQualityMetrics = (sprintData) => {
  ...
  const metrics = sprintData.reduce((acc, sprint) => {
  ...
  if (sprintData.length >= 3) {
    ...
    const firstAvg = firstHalf.reduce((sum, b) => sum + b, 0) / firstHalf.length;
    const secondAvg = secondHalf.reduce((sum, b) => sum + b, 0) / secondHalf.length;
...
export const generateActionableInsights = (retrospectiveData) => {
  ...
  const totalActionItems = recentRetros.reduce((sum, retro) => 
  ...
  if (totalActionItems > recentRetros.length * 3) {
    insights.push({
      ...
      recommendation: 'Focus on 1-2 high-impact action items per sprint for better follow-through'
    });
  }
  ...
  if (satisfactionMetrics.satisfactionTrend === 'declining') {
    insights.push({
      ...
      description: 'Team satisfaction scores have been declining over recent sprints',
      recommendation: 'Schedule a focused team discussion to address underlying concerns'
    });
  }
