...
const BorrowingDashboard = () => {
  ...
  const fetchTransactions = async () => {
    try {
      ...
      const activeResponse = await fetch('/api/v1/lending-transactions/my-transactions/list?status=active', {
        headers: {
      ...
      const overdueResponse = await fetch('/api/v1/lending-transactions/my-transactions/list?status=overdue', {
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('token')}`
        }
      });
      ...
      // Fetch history
      const historyResponse = await fetch('/api/v1/lending-transactions/history?limit=20', {
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('token')}`
        }
      });
      ...
      <Card className={`${isOverdue ? 'border-red-200 bg-red-50' : isDueSoon ? 'border-yellow-200 bg-yellow-50' : ''}`}>
        <CardHeader className="pb-3">
          ...
          <div className="flex items-center gap-2 text-sm text-gray-600">
            <span className="font-medium">{transaction.productBrand || transaction.brand}</span>
            {transaction.productModel && <span>• {transaction.productModel}</span>}
          </div>
        </CardHeader>
        ...
        <CardContent className="space-y-4">
          <div className="grid grid-cols-2 gap-4 text-sm">
            ...
            <div className="flex items-center gap-2">
              ...
              <div>
                ...
                <div className={`${isOverdue ? 'text-red-600 font-medium' : isDueSoon ? 'text-yellow-600 font-medium' : 'text-gray-600'}`}>
          ...
          {transaction.returnDate && (
            <div className="flex items-center gap-2 text-sm text-green-600">
              ...
              <span>Returned on {new Date(transaction.returnDate).toLocaleDateString()}</span>
            </div>
          )}
  ...
  const dueSoonCount = activeTransactions.filter(t => getDaysUntilDue(t.dueDate) <= 3).length;

  return (
    <div className="container mx-auto px-4 py-8">
      <div className="mb-8">
        <h1 className="text-3xl font-bold mb-2">My Borrowing Dashboard</h1>
        <p className="text-gray-600">Manage your borrowed items and view your lending history</p>
      </div>
      ...
      {totalOverdue > 0 && (
        <Alert className="mb-6 border-red-200 bg-red-50">
          ...
          <AlertDescription className="text-red-800">
            You have {totalOverdue} overdue item(s). Please return them as soon as possible to avoid penalties.
          </AlertDescription>
        </Alert>
      )}
        ...
        <TabsContent value="active" className="mt-6">
          {activeTransactions.length > 0 ? (
            ...
            <div className="text-center py-12">
              ...
              <p className="text-gray-600 mb-4">You don't have any items currently borrowed.</p>
        ...
        <TabsContent value="overdue" className="mt-6">
          {overdueTransactions.length > 0 ? (
            ...
            <div className="text-center py-12">
              ...
              <p className="text-gray-600">Great job! You don't have any overdue items.</p>
            </div>
          )}
        </TabsContent>
