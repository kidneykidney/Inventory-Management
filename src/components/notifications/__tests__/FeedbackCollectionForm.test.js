...
describe('FeedbackCollectionForm', () => {
  ...
  test('renders feedback form trigger button', () => {
    ...
    expect(
      screen.getByRole('button', { name: /provide feedback/i })
    ).toBeInTheDocument();
  });
  ...
  test('submits feedback with valid data', async () => {
    ...
    await user.type(
      ...
      'The team worked well together and delivered all planned features.'
    );
  ...
  test('handles star rating interaction', async () => {
    ...
    // Click on third star
    const stars = screen.getAllByRole('button');
    const starButtons = stars.filter(
      button =>
  ...
  test('handles API errors during submission', async () => {
    const user = userEvent.setup();
    notificationService.submitFeedback.mockRejectedValue(
      new Error('API Error')
    );
