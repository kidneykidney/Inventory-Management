...
const AnalyticsDashboard = () => {
  ...
  const calculatePerformanceMetrics = (sprints, retrospectives) => {
    ...
    const totalVelocity = recentSprints.reduce(
      (sum, sprint) => sum + (sprint.velocity || 0),
      0
    );
    ...
    const satisfactionScores = retrospectives
      .slice(-6)
      .map(
        retro =>
          ((retro.teamMorale || 0) +
            (retro.velocityRating || 0) +
            (retro.qualityRating || 0) +
            (retro.communicationRating || 0)) /
          4
      );
    ...
    const totalActionItems = retrospectives
      .slice(-6)
      .reduce(
        (sum, retro) =>
          sum + (retro.actionItems ? retro.actionItems.length : 0),
        0
      );
            ...
            <div className='flex items-center justify-between'>
              <div>
                ...
                <p className='text-2xl font-bold'>
                  {performanceMetrics.averageVelocity || 0}
                </p>
          ...
          <CardContent className='p-6'>
            <div className='flex items-center justify-between'>
              <div>
                ...
                <p className='text-2xl font-bold'>
                  {performanceMetrics.averageCompletion || 0}%
                </p>
          ...
          <CardContent className='p-6'>
            <div className='flex items-center justify-between'>
              <div>
                ...
                <p className='text-2xl font-bold'>
                  {performanceMetrics.averageSatisfaction || 0}
                </p>
          ...
          <CardContent className='p-6'>
            <div className='flex items-center justify-between'>
              <div>
                ...
                <p className='text-2xl font-bold'>
                  {performanceMetrics.totalActionItems || 0}
                </p>
          ...
          <div className='grid grid-cols-1 md:grid-cols-3 gap-6'>
            ...
            <div className='text-center'>
              <div className='text-3xl font-bold text-purple-600 mb-2'>
                {sprintData.length > 0
                  ? Math.round(
                      (Date.now() - new Date(sprintData[0].startDate)) /
                        (1000 * 60 * 60 * 24 * 7)
                    )
                  : 0}
              </div>
              ...
              >
                <div>
                  <p className='font-medium'>
                    Sprint {retro.sprintId} Retrospective
                  </p>
                  <p className='text-sm text-gray-600'>
                    {retro.actionItems?.length || 0} action items, Team morale:{' '}
                    {retro.teamMorale || 0}/10
                  </p>
                </div>
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className='w-full'>
        ...
        <TabsContent value='retrospective' className='space-y-4'>
          <SprintRetrospectiveForm
            sprintId={
              sprintData.length > 0
                ? sprintData[sprintData.length - 1].number
                : 1
            }
