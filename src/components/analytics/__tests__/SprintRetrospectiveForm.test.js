...
describe('SprintRetrospectiveForm', () => {
  ...
  it('renders form with all required fields', () => {
    ...
    expect(
      screen.getByText('Sprint Retrospective - Sprint 5')
    ).toBeInTheDocument();
  ...
  it('allows adding and removing "What Went Well" items', async () => {
    ...
    const textareas = screen.getAllByPlaceholderText(
      'Describe something that went well...'
    );
  ...
  it('allows adding and removing action items', async () => {
    ...
    const assigneeInputs = screen.getAllByPlaceholderText(
  ...
  it('submits form with correct data structure', async () => {
    ...
    // Fill in form data
    const whatWentWellTextarea = screen.getByPlaceholderText(
      'Describe something that went well...'
    );
    await user.type(whatWentWellTextarea, 'Great sprint planning');

    const whatCouldImproveTextarea = screen.getByPlaceholderText(
      'Describe something that could be improved...'
    );
    await user.type(whatCouldImproveTextarea, 'Better communication');

    const actionItemTextarea = screen.getByPlaceholderText(
      'Describe the action item...'
    );
    ...
    const additionalNotesTextarea = screen.getByPlaceholderText(
  ...
  it('renders with initial data when provided', () => {
    ...
    render(
      <SprintRetrospectiveForm {...defaultProps} initialData={initialData} />
    );
    ...
    expect(
      screen.getByDisplayValue('Improve test coverage')
    ).toBeInTheDocument();
    expect(screen.getByDisplayValue('QA Team')).toBeInTheDocument();
    expect(
      screen.getByDisplayValue('Great sprint overall')
    ).toBeInTheDocument();
  });
