...
const SprintRetrospectiveForm = ({ sprintId, onSubmit, initialData = null }) => {
  const [formData, setFormData] = useState({
    sprintId: sprintId || '',
    whatWentWell: initialData?.whatWentWell || [''],
    whatCouldImprove: initialData?.whatCouldImprove || [''],
    actionItems: initialData?.actionItems || [{ description: '', assignee: '', priority: 'medium' }],
  ...
  const addItem = (field) => {
    setFormData(prev => ({
      ...prev,
      [field]: [...prev[field], field === 'actionItems' ? { description: '', assignee: '', priority: 'medium' } : '']
    }));
  };
    ...
    <div className="space-y-2">
      ...
      <div className="flex items-center space-x-2">
        <Input
          ...
          className="flex-1"
        />
        <Badge variant="outline" className="w-8 text-center">{value}</Badge>
      </div>
    </div>
  );
      ...
      <CardContent>
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="space-y-3">
            <Label className="text-lg font-semibold text-green-700">What Went Well</Label>
          ...
          {/* What Could Improve */}
          <div className="space-y-3">
            <Label className="text-lg font-semibold text-orange-700">What Could Improve</Label>
          ...
          {/* Action Items */}
          <div className="space-y-3">
            <Label className="text-lg font-semibold text-blue-700">Action Items</Label>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <RatingInput
              ...
              onChange={(value) => setFormData(prev => ({ ...prev, teamMorale: value }))}
            />
            <RatingInput
              label="Velocity Rating"
              value={formData.velocityRating}
              onChange={(value) => setFormData(prev => ({ ...prev, velocityRating: value }))}
            />
            <RatingInput
            ...
            <RatingInput
              ...
              onChange={(value) => setFormData(prev => ({ ...prev, communicationRating: value }))}
            />
          </div>
