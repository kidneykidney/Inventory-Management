...
const TeamPerformanceDashboard = ({ teamData, sprintData, retrospectiveData }) => {
  ...
  const calculateAverageVelocity = () => {
    ...
    const totalVelocity = recentSprints.reduce((sum, sprint) => sum + (sprint.completedPoints || 0), 0);
  ...
  const calculateVelocityTrend = () => {
    ...
    return recentSprints.map((sprint, index) => ({
      ...
      trend: index > 0 ? sprint.velocity - recentSprints[index - 1].velocity : 0
    }));
  };
  ...
  const calculateStoryCompletionRate = () => {
    ...
    return recentSprints.map(sprint => ({
      ...
      completed: sprint.stories?.filter(story => story.status === 'done').length || 0,
      completionRate: sprint.stories?.length ? 
        Math.round((sprint.stories.filter(story => story.status === 'done').length / sprint.stories.length) * 100) : 0
    }));
  };
  ...
  const calculateTeamSatisfaction = () => {
    ...
    return retrospectiveData.slice(-6).map(retro => ({
      ...
      average: Math.round(((retro.teamMorale + retro.velocityRating + retro.qualityRating + retro.communicationRating) / 4) * 10) / 10
    }));
  };

  const calculatePredictiveVelocity = () => {
    const recentVelocities = sprintData.slice(-6).map(s => s.velocity || 0);
    const trend = recentVelocities.slice(-3).reduce((sum, v, i, arr) => {
      if (i === 0) return 0;
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        <MetricCard
          ...
          trend={performanceMetrics.velocityTrend?.slice(-2).reduce((acc, curr, i, arr) => 
            i === 1 ? curr.velocity - arr[0].velocity : acc, 0)}
          icon={Target}
          color="blue"
        />
        <MetricCard
        ...
        <MetricCard
          ...
          value={performanceMetrics.teamSatisfaction?.slice(-1)[0]?.average || 0}
          icon={Users}
          color="orange"
        />
      </div>
            ...
            <CardContent>
              <ResponsiveContainer width="100%" height={300}>
                <LineChart data={performanceMetrics.velocityTrend || []}>
                  ...
                  <Tooltip />
                  <Legend />
                  <Line type="monotone" dataKey="planned" stroke="#8884d8" name="Planned Points" />
                  <Line type="monotone" dataKey="completed" stroke="#82ca9d" name="Completed Points" />
                  <Line type="monotone" dataKey="velocity" stroke="#ffc658" name="Velocity" strokeWidth={3} />
                </LineChart>
              </ResponsiveContainer>
            </CardContent>
          </Card>
              ...
              <CardContent>
                <ResponsiveContainer width="100%" height={250}>
                  <AreaChart data={performanceMetrics.qualityMetrics || []}>
                    ...
                    <YAxis />
                    <Tooltip />
                    <Area type="monotone" dataKey="testCoverage" stackId="1" stroke="#8884d8" fill="#8884d8" name="Test Coverage %" />
                    <Area type="monotone" dataKey="codeReviewScore" stackId="2" stroke="#82ca9d" fill="#82ca9d" name="Code Review Score" />
                  </AreaChart>
                </ResponsiveContainer>
              </CardContent>
            </Card>
              ...
              <CardContent>
                <ResponsiveContainer width="100%" height={250}>
                  <BarChart data={performanceMetrics.qualityMetrics || []}>
                    ...
                    <Bar dataKey="technicalDebt" fill="#413ea0" name="Technical Debt (hrs)" />
                  </BarChart>
                </ResponsiveContainer>
              </CardContent>
            </Card>
          ...
          <Card>
            ...
            <CardContent>
              <ResponsiveContainer width="100%" height={400}>
                <RadarChart data={performanceMetrics.teamSatisfaction?.slice(-1) || []}>
                  <PolarGrid />
                  <PolarAngleAxis dataKey="category" />
                  <PolarRadiusAxis angle={90} domain={[0, 10]} />
                  <Radar name="Current Sprint" dataKey="morale" stroke="#8884d8" fill="#8884d8" fillOpacity={0.6} />
                  <Radar name="Velocity" dataKey="velocity" stroke="#82ca9d" fill="#82ca9d" fillOpacity={0.6} />
            ...
            <CardContent>
              <ResponsiveContainer width="100%" height={300}>
                <BarChart data={performanceMetrics.storyCompletionRate || []}>
                  ...
                  <YAxis />
                  <Tooltip />
                  <Bar dataKey="planned" fill="#8884d8" name="Planned Stories" />
                  <Bar dataKey="completed" fill="#82ca9d" name="Completed Stories" />
                </BarChart>
              </ResponsiveContainer>
              <div className="mt-4 space-y-2">
                {performanceMetrics.storyCompletionRate?.map((sprint, index) => (
                  <div key={index} className="flex items-center justify-between">
                    <span className="text-sm font-medium">{sprint.sprint}</span>
                    <div className="flex items-center space-x-2">
                      <Progress value={sprint.completionRate} className="w-24" />
                      <Badge variant={sprint.completionRate >= 80 ? "default" : "secondary"}>
            ...
            <Card>
              ...
              <CardContent>
                <div className="text-center">
                  ...
                  <p className="text-gray-600">Predicted points for next sprint</p>
                  <div className="mt-4 p-4 bg-blue-50 rounded-lg">
                    <p className="text-sm text-blue-800">
                      Based on recent velocity trends and team performance patterns
                    </p>
                  </div>
                </div>
              </CardContent>
                ...
                <div className="space-y-4">
                  ...
                  <div className="flex justify-between items-center">
                    ...
                    <Badge variant="outline">
                      {Math.round((performanceMetrics.predictiveVelocity || 0) * 2.5)} hrs
                    </Badge>
                  </div>
                  <Progress 
                    value={Math.min(100, ((performanceMetrics.predictiveVelocity || 0) * 2.5) / (teamData?.capacity || 1) * 100)} 
                    className="w-full"
                  />
                  <p className="text-xs text-gray-600">
                    Utilization: {Math.round(((performanceMetrics.predictiveVelocity || 0) * 2.5) / (teamData?.capacity || 1) * 100)}%
                  </p>
                </div>
              </CardContent>
            </Card>
