  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        throw new Error(
          `Failed to ${selectedTemplate ? 'update' : 'create'} template`
        );
      }
    } catch (error) {
      toast({
    ...
    return (
      <Card>
        ...
        <CardContent className='space-y-6'>
          ...
          <Alert>
            <AlertDescription>
              Available variables: {`{borrowerName}`}, {`{productName}`},{' '}
              {`{lendDate}`}, {`{dueDate}`}, {`{returnDate}`}, {`{condition}`},{' '}
              {`{lendingId}`}, {`{daysUntilDue}`}, {`{daysOverdue}`},{' '}
              {`{approverName}`}
            </AlertDescription>
          </Alert>
