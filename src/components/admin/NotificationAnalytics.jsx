...
import { RefreshCw, Mail, AlertTriangle, CheckCircle, XCircle, Clock } from 'lucide-react';
...
const NotificationAnalytics = () => {
  ...
  const fetchStats = async () => {
    ...
    const response = await fetch(`/api/v1/email-notifications/stats?days=${selectedPeriod}`, {
      headers: {
  ...
  const fetchFailedNotifications = async () => {
    const token = localStorage.getItem('inventory_auth_token');
    const response = await fetch('/api/v1/email-notifications/failed?limit=50', {
      headers: {
        'Authorization': `Bearer ${token}`,
        'Content-Type': 'application/json'
      }
  ...
  const fetchSchedulerStatus = async () => {
    const token = localStorage.getItem('inventory_auth_token');
    const response = await fetch('/api/v1/email-notifications/scheduler/status', {
      headers: {
        'Authorization': `Bearer ${token}`,
        'Content-Type': 'application/json'
      }
  ...
  const handleRetryNotification = async (notificationId) => {
    ...
    try {
      const token = localStorage.getItem('inventory_auth_token');
      const response = await fetch(`/api/v1/email-notifications/retry/${notificationId}`, {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
  ...
  const handleSchedulerAction = async (action) => {
    try {
      const token = localStorage.getItem('inventory_auth_token');
      const response = await fetch(`/api/v1/email-notifications/scheduler/${action}`, {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
          ...
          <Card>
            <CardContent className="p-6">
              <div className="flex items-center space-x-2">
                ...
                <div>
                  <p className="text-2xl font-bold">{stats.summary.total_notifications || 0}</p>
          ...
          <Card>
            <CardContent className="p-6">
              <div className="flex items-center space-x-2">
                ...
                <div>
                  <p className="text-2xl font-bold">{stats.summary.sent_count || 0}</p>
          ...
          <Card>
            <CardContent className="p-6">
              <div className="flex items-center space-x-2">
                ...
                <div>
                  <p className="text-2xl font-bold">{stats.summary.failed_count || 0}</p>
          ...
          <Card>
            <CardContent className="p-6">
              <div className="flex items-center space-x-2">
                ...
                <div>
                  <p className="text-2xl font-bold">{stats.summary.success_rate || 0}%</p>
              ...
              <div className="space-y-2">
                <div className="flex items-center space-x-2">
                  ...
                  <span className="text-sm text-gray-500">
                    Active Jobs: {schedulerStatus.activeJobs?.join(', ') || 'None'}
                  </span>
                </div>
                {schedulerStatus.nextReminderRun && (
                  <p className="text-sm text-gray-500">
                    Next Reminder Run: {formatDate(schedulerStatus.nextReminderRun)}
                  </p>
                )}
                {schedulerStatus.nextOverdueRun && (
                  <p className="text-sm text-gray-500">
                    Next Overdue Run: {formatDate(schedulerStatus.nextOverdueRun)}
                  </p>
                )}
              </div>
