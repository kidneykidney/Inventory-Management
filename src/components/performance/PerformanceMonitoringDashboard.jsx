    ...
    <div className='p-6 space-y-6'>
      ...
      <div className='flex items-center justify-between'>
        ...
        <div className='flex gap-2'>
          ...
          <Button size='sm' onClick={runLoadTest} disabled={loadTestRunning}>
            <Play
              className={`h-4 w-4 mr-2 ${loadTestRunning ? 'animate-pulse' : ''}`}
            />
      ...
      <div>
        ...
        <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6'>
          <Card>
            ...
            <CardContent>
              <div
                className={`text-2xl font-bold ${getStatusColor(backend.requests.avgResponseTime, { warning: 1000, critical: 2000 })}`}
              >
              ...
              <div className='flex items-center gap-2 mt-2'>
                <Progress
                  value={Math.min(
                    (backend.requests.avgResponseTime / 2000) * 100,
                    100
                  )}
                  className='flex-1'
                />
                <Badge
                  className={getStatusBadge(backend.requests.avgResponseTime, {
                    warning: 1000,
                    critical: 2000,
                  })}
                >
                  {backend.requests.avgResponseTime < 1000
                    ? 'Good'
                    : backend.requests.avgResponseTime < 2000
                      ? 'Warning'
                      : 'Critical'}
                </Badge>
              </div>
          ...
          <Card>
            ...
            <CardContent>
              <div
                className={`text-2xl font-bold ${getStatusColor(backend.system.memoryUsage, { warning: 512, critical: 1024 })}`}
              >
              ...
              <div className='flex items-center gap-2 mt-2'>
                <Progress
                  value={Math.min(
                    (backend.system.memoryUsage / 1024) * 100,
                    100
                  )}
                  className='flex-1'
                />
                <Badge
                  className={getStatusBadge(backend.system.memoryUsage, {
                    warning: 512,
                    critical: 1024,
                  })}
                >
                  {backend.system.memoryUsage < 512
                    ? 'Good'
                    : backend.system.memoryUsage < 1024
                      ? 'Warning'
                      : 'Critical'}
                </Badge>
              </div>
              <p className='text-xs text-muted-foreground mt-2'>
                Uptime: {Math.floor(backend.system.uptime / 3600)}h{' '}
                {Math.floor((backend.system.uptime % 3600) / 60)}m
              </p>
            </CardContent>
          </Card>
          ...
          <Card>
            ...
            <CardContent>
              ...
              <div className='flex items-center gap-2 mt-2'>
                ...
                <Badge
                  className={getStatusBadge(backend.database.slowQueries, {
                    warning: 5,
                    critical: 20,
                  })}
                >
                  {backend.database.slowQueries < 5
                    ? 'Good'
                    : backend.database.slowQueries < 20
                      ? 'Warning'
                      : 'Critical'}
                </Badge>
              </div>
      ...
      <div>
        ...
        <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6'>
          <Card>
            ...
            <CardContent>
              <div
                className={`text-2xl font-bold ${getStatusColor(frontend.pageLoads.avgLoadTime, { warning: 3000, critical: 5000 })}`}
              >
              ...
              <div className='flex items-center gap-2 mt-2'>
                <Progress
                  value={Math.min(
                    (frontend.pageLoads.avgLoadTime / 5000) * 100,
                    100
                  )}
                  className='flex-1'
                />
                <Badge
                  className={getStatusBadge(frontend.pageLoads.avgLoadTime, {
                    warning: 3000,
                    critical: 5000,
                  })}
                >
                  {frontend.pageLoads.avgLoadTime < 3000
                    ? 'Good'
                    : frontend.pageLoads.avgLoadTime < 5000
                      ? 'Warning'
                      : 'Critical'}
                </Badge>
              </div>
                  ...
                  <Badge
                    className={getStatusBadge(
                      frontend.coreWebVitals.lcp.count > 0
                        ? (frontend.coreWebVitals.lcp.goodCount /
                            frontend.coreWebVitals.lcp.count) *
                            100
                        : 100,
                  ...
                  <Badge
                    className={getStatusBadge(
                      frontend.coreWebVitals.fid.count > 0
                        ? (frontend.coreWebVitals.fid.goodCount /
                            frontend.coreWebVitals.fid.count) *
                            100
                        : 100,
                      { warning: 75, critical: 50 }
                    )}
                  ...
                  <Badge
                    className={getStatusBadge(
                      frontend.coreWebVitals.cls.count > 0
                        ? (frontend.coreWebVitals.cls.goodCount /
                            frontend.coreWebVitals.cls.count) *
                            100
                        : 100,
                      { warning: 75, critical: 50 }
                    )}
        ...
        <CardContent>
          ...
          backend.database.slowQueries > 5 ? (
            <div className='space-y-3'>
              {backend.requests.avgResponseTime > 1000 && (
                <Alert className='border-yellow-200 bg-yellow-50'>
                  ...
                  <AlertDescription>
                    ...
                    {backend.requests.avgResponseTime}ms). Consider optimizing
                    API endpoints or scaling infrastructure.
                  </AlertDescription>
                </Alert>
              )}
              {backend.system.memoryUsage > 512 && (
                <Alert className='border-orange-200 bg-orange-50'>
                  <AlertTriangle className='h-4 w-4' />
                  <AlertDescription>
                    High memory usage detected ({backend.system.memoryUsage}MB).
                    Monitor for memory leaks and consider increasing server
                    capacity.
                  </AlertDescription>
                </Alert>
              )}
              {backend.database.slowQueries > 5 && (
                <Alert className='border-red-200 bg-red-50'>
                  <AlertTriangle className='h-4 w-4' />
                  <AlertDescription>
                    Multiple slow database queries detected (
                    {backend.database.slowQueries}). Review and optimize
            ...
            <div className='flex items-center gap-2 text-green-600'>
              ...
              <span>
                No performance issues detected. System is operating within
                normal parameters.
              </span>
            </div>
          )}
