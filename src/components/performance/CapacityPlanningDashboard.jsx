...
const CapacityPlanningDashboard = () => {
  ...
  const fetchCapacityData = async () => {
    try {
      setLoading(true);
      const response = await fetch(`/api/v1/performance/capacity-planning?timeframe=${selectedTimeframe}`);
      if (!response.ok) {
      ...
      <div className="p-6 space-y-6">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-gray-600 mt-1">Monitor system capacity and plan for scaling</p>
          </div>
        </div>
    ...
    <div className="p-6 space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">{capacityData.cpu.current}%</div>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">{capacityData.memory.current}%</div>
            <div className="flex items-center gap-2 mt-2">
              <Progress value={capacityData.memory.current} className="flex-1" />
              {getTrendIcon(capacityData.memory.trend)}
            </div>
            <p className="text-xs text-muted-foreground mt-2">
              Peak: {capacityData.memory.peak}% | Avg: {capacityData.memory.average}%
            </p>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">{capacityData.users.current}</div>
            <div className="flex items-center gap-2 mt-2">
              <Progress 
                value={(capacityData.users.current / capacityData.users.capacity) * 100} 
                className="flex-1" 
              />
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">{capacityData.responseTime.current}ms</div>
            <div className="flex items-center gap-2 mt-2">
              <Progress 
                value={Math.min((capacityData.responseTime.current / 2000) * 100, 100)} 
                className="flex-1" 
              />
                ...
                <div key={index} className="border rounded-lg p-4">
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="flex justify-between text-sm">
                      ...
                      <span className="font-medium text-blue-600">{item.recommended}</span>
                    </div>
                  </div>
                  <Progress 
                    value={(parseFloat(item.projected) / parseFloat(item.recommended)) * 100} 
                    className="mt-3" 
                  />
                </div>
              ))}
