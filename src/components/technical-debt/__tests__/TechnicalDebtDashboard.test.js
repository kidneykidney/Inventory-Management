...
describe('TechnicalDebtDashboard', () => {
  test('renders dashboard with header', () => {
    ...
    expect(
      screen.getByText('Track and manage technical debt across the codebase')
    ).toBeInTheDocument();
    expect(
      screen.getByRole('button', { name: /add technical debt/i })
    ).toBeInTheDocument();
  });
  ...
  test('shows technical debt items in table', () => {
    ...
    expect(
      screen.getByText('Migrate Material-UI to Shadcn UI')
    ).toBeInTheDocument();
    expect(
      screen.getByText('Implement proper error handling in API routes')
    ).toBeInTheDocument();
    expect(
      screen.getByText('Add comprehensive unit tests for utility functions')
    ).toBeInTheDocument();
  });
  ...
  test('filters items by category', async () => {
    ...
    const uiFrameworkOption = screen.getByRole('option', {
      name: 'UI Framework',
    });
    ...
    expect(
      screen.queryByText('Implement proper error handling in API routes')
  ...
  test('shows category breakdown', () => {
    ...
    expect(
      screen.getByText('Distribution of technical debt across categories')
    ).toBeInTheDocument();
  });
  ...
  test('shows priority distribution', () => {
    ...
    expect(
      screen.getByText('Technical debt items by priority level')
    ).toBeInTheDocument();
  });
