  ...
  const fetchSuggestions = useCallback((searchQuery) => {
    ...
    debounceTimeoutRef.current = setTimeout(() => {
      if (searchQuery.length >= 2) {
        const newSuggestions = searchService.getSuggestions(products, searchQuery, 5);
        setSuggestions(newSuggestions);
        setShowSuggestionDropdown(true);
        setSelectedSuggestionIndex(-1);
      } else {
        setSuggestions({ products: [], brands: [], tags: [], categories: [] });
    ...
    switch (e.key) {
      ...
      case 'Enter':
        ...
        if (selectedSuggestionIndex >= 0 && allSuggestions[selectedSuggestionIndex]) {
          ...
          handleSuggestionSelect(suggestion.name || suggestion, suggestion.type);
        } else {
          handleSearch();
        }
        break;
                ...
                <div data-testid="brand-suggestions">
                  {suggestions.brands.map((suggestion, index) => 
                    renderSuggestionItem(
                      ...
                      (index + suggestions.products.length) === selectedSuggestionIndex
                    )
                  )}
                </div>
                <div data-testid="tag-suggestions">
                  {suggestions.tags.map((suggestion, index) => 
                    renderSuggestionItem(
                      suggestion, 
                      'tag', 
                      index + suggestions.products.length + suggestions.brands.length, 
                      (index + suggestions.products.length + suggestions.brands.length) === selectedSuggestionIndex
                    )
                  )}
                </div>
              </div>
        ...
        <Card className="mt-2">
          ...
          <CardContent>
            <div className="text-sm text-gray-600">
              Advanced filtering options will be implemented here based on your specific requirements.
              This could include date ranges, price ranges, specific attributes, etc.
            </div>
          </CardContent>
        </Card>
      )}
