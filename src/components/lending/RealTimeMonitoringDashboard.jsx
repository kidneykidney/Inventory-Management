...
import { 
  Activity, Wifi, WifiOff, RefreshCw, AlertTriangle, CheckCircle, 
  Clock, Users, Package, TrendingUp, TrendingDown, Zap
} from 'lucide-react';
...
const RealTimeMonitoringDashboard = () => {
  ...
  const fetchRealtimeMetrics = async () => {
    try {
      ...
      const response = await fetch(`/api/v1/lending-analytics/usage-statistics?startDate=${today}&endDate=${today}`, {
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('token')}`
        }
      });
  ...
  const generateAlerts = (data) => {
    ...
    const returnRate = data.total_lendings > 0 ? (data.returned_count / data.total_lendings) * 100 : 0;
    ...
    <div className="p-6 space-y-6">
      ...
      <div className="flex justify-between items-center">
        ...
        <div className="flex items-center space-x-4">
          ...
          <Button
            onClick={isConnected ? stopRealTimeMonitoring : startRealTimeMonitoring}
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <Card>
          ...
          <CardContent>
            <div className="flex items-center space-x-2">
              ...
              {healthStatus.status === 'poor' && <AlertTriangle className="h-4 w-4 text-red-500" />}
            </div>
        ...
        <Card>
          ...
          <CardContent>
            ...
            <Progress value={systemHealth?.system.return_rate || 0} className="mt-2" />
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            ...
            <Progress value={systemHealth?.system.on_time_return_rate || 0} className="mt-2" />
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground">
              {systemHealth?.response.same_day_return_rate?.toFixed(1) || 0}% same day
            </p>
          </CardContent>
        </Card>
      </div>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Live lending activity over the last 10 minutes</CardDescription>
        </CardHeader>
        <CardContent>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        <Card>
          <CardHeader>
            ...
            <CardDescription>Real-time inventory turnover by category</CardDescription>
          </CardHeader>
          <CardContent>
            {systemHealth?.inventoryTurnover ? (
              <ResponsiveContainer width="100%" height={250}>
                <BarChart data={systemHealth.inventoryTurnover}>
                  ...
                  <YAxis />
                  <Tooltip />
                  <Bar dataKey="utilization_rate" fill="#8884d8" name="Utilization %" />
                </BarChart>
              </ResponsiveContainer>
            ) : (
              <div className="flex items-center justify-center h-64 text-muted-foreground">
        ...
        <Card>
          ...
          <CardContent>
            <div className="space-y-4">
              <div className="flex justify-between items-center">
                <span className="text-sm font-medium">Average Lending Duration</span>
                <Badge variant="outline">
                  {systemHealth?.system.avg_lending_duration?.toFixed(1) || 0} days
                </Badge>
              </div>
              <div className="flex justify-between items-center">
                <span className="text-sm font-medium">Average Overdue Days</span>
      ...
      {error && (
        <Alert>
          ...
          <AlertDescription>
            ...
            <Button onClick={startRealTimeMonitoring} variant="outline" size="sm" className="ml-2">
              Reconnect
            </Button>
          </AlertDescription>
        </Alert>
