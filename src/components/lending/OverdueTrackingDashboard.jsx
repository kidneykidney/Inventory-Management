...
import { 
  AlertTriangle, Clock, Mail, Phone, User, Package, 
  Calendar, Filter, Search, RefreshCw, Send
} from 'lucide-react';
...
const OverdueTrackingDashboard = () => {
  ...
  const fetchOverdueItems = async () => {
    try {
      ...
      const response = await fetch('/api/v1/lending-analytics/overdue-tracking', {
        headers: {
  ...
  const sendReminder = async (transactionId) => {
    try {
      const response = await fetch(`/api/v1/email-notifications/send-reminder`, {
        method: 'POST',
        headers: {
    ...
    return (
      <Alert className="m-4">
        ...
        <AlertDescription>
          ...
          <Button onClick={fetchOverdueItems} variant="outline" size="sm" className="ml-2">
            Retry
          </Button>
        </AlertDescription>
      </Alert>
    ...
    <div className="p-6 space-y-6">
                        ...
                        <div className="flex items-center space-x-2">
                          ...
                          {item.last_reminder_sent && (
                            <span className="text-xs text-muted-foreground">
                              (Last: {new Date(item.last_reminder_sent).toLocaleDateString()})
                            </span>
                          )}
                        </div>
                      </TableCell>
                          ...
                          {item.escalation_level === 'Critical' && (
                            <Button
                              ...
                              onClick={() => {
                                ...
                                window.open(`mailto:${item.borrower_email}?subject=URGENT: Overdue Item Return Required&body=Your borrowed item "${item.product_name}" is ${item.days_overdue} days overdue. Please return immediately.`);
                              }}
                            >
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Automated escalation process for overdue items</CardDescription>
        </CardHeader>
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
