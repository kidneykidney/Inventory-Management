  ...
  const getActiveFilterCount = () => {
    ...
    Object.entries(localFilters).forEach(([key, value]) => {
      if (value && value !== 'all') {
        ...
        else if (typeof value === 'object' && (value.start || value.end)) count++;
      ...
      {items.map((item, index) => (
        <label key={index} className="flex items-center gap-2 cursor-pointer hover:bg-gray-50 p-1 rounded">
          ...
          <span className="text-xs text-gray-500">
            ({products.filter(p => p[filterKey.slice(0, -1)] === item || (Array.isArray(p[filterKey.slice(0, -1)]) && p[filterKey.slice(0, -1)].includes(item))).length})
          </span>
        </label>
      ))}
    </div>
      ...
      {items.map((item, index) => (
        <label key={index} className="flex items-center gap-2 cursor-pointer hover:bg-gray-50 p-1 rounded">
          <input
            ...
            className="text-blue-600 focus:ring-blue-500"
          />
          <span className="text-sm text-gray-700 flex-1">{item.label || item}</span>
          {item.count && (
  ...
  return (
    <div className={`bg-white border-r border-gray-200 ${className}`}>
      ...
      <div className="overflow-y-auto max-h-[calc(100vh-120px)]">
        {/* Category Filter */}
        {renderSection('Category', 'category', Package, (
          renderRadioList([
            { value: 'electronics', label: 'Electronics', count: products.filter(p => p.category === 'electronics').length },
            { value: 'office_supplies', label: 'Office Supplies', count: products.filter(p => p.category === 'office_supplies').length },
        ...
        {/* Availability Filter */}
        {renderSection('Availability', 'availability', Package, (
          renderRadioList([
            { value: 'available', label: 'Available', count: products.filter(p => p.isAvailable).length },
            { value: 'unavailable', label: 'Unavailable', count: products.filter(p => !p.isAvailable).length }
          ], 'availability', localFilters.availability)
        ))}

        {/* Condition Filter */}
        {renderSection('Condition', 'condition', Star, (
          renderRadioList([
            { value: 'excellent', label: 'Excellent', count: products.filter(p => p.conditionStatus === 'excellent').length },
            { value: 'good', label: 'Good', count: products.filter(p => p.conditionStatus === 'good').length },
            { value: 'fair', label: 'Fair', count: products.filter(p => p.conditionStatus === 'fair').length },
            { value: 'needs_repair', label: 'Needs Repair', count: products.filter(p => p.conditionStatus === 'needs_repair').length }
          ], 'condition', localFilters.condition)
        ))}

        {/* Brand Filter */}
        {filterSuggestions.brands.length > 0 && renderSection('Brand', 'brand', Building, (
          renderCheckboxList(filterSuggestions.brands, 'brands', localFilters.brand || [])
        ))}

        {/* Location Filter */}
        {renderSection('Location', 'location', MapPin, (
            ...
            {filterSuggestions.locations.length > 0 && (
              <div className="space-y-1">
                ...
                <div className="flex flex-wrap gap-1">
                  {filterSuggestions.locations.slice(0, 6).map((location, index) => (
                    <Badge
                      key={index}
                      variant="outline"
        ...
        {/* Tags Filter */}
        {filterSuggestions.tags.length > 0 && renderSection('Tags', 'tags', Tag, (
          <div className="space-y-3">
        ...
        {/* Date Range Filter */}
        {renderSection('Date Added', 'dateRange', Calendar, (
          <div className="space-y-3">
            <div>
        ...
        {renderSection('Max Lending Period', 'lendingPeriod', Clock, (
          <div className="space-y-3">
            <div>
              ...
              <input
                ...
                onChange={(e) => updateFilter('maxLendingPeriod', Number(e.target.value))}
