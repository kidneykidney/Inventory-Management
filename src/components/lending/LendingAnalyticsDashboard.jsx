...
const LendingAnalyticsDashboard = () => {
  ...
  const fetchAnalyticsData = async () => {
    try {
      ...
      const response = await fetch(
        `/api/v1/lending-analytics/report?includeDetails=true`,
        {
          headers: {
  ...
  const exportReport = async (format = 'json') => {
    try {
      const response = await fetch(
        `/api/v1/lending-analytics/report?format=${format}`,
        {
          headers: {
            Authorization: `Bearer ${localStorage.getItem('token')}`,
          },
    ...
    <div className='p-6 space-y-6'>
      ...
      <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4'>
        ...
        <Card>
          ...
          <CardContent>
            <div className='text-2xl font-bold'>
              {overview.products.available_products}
            </div>
            <p className='text-xs text-muted-foreground'>
        ...
        <Card>
          ...
          <CardContent>
            <div className='text-2xl font-bold'>
              {overview.utilization.utilization_rate}%
            </div>
            <Progress
              value={overview.utilization.utilization_rate}
              className='mt-2'
            />
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            ...
            <p className='text-xs text-muted-foreground'>
              Avg {overview.transactions.avg_lending_period?.toFixed(1)} days
              lending
            </p>
          </CardContent>
        </Card>
        ...
        <TabsContent value='trends' className='space-y-4'>
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Monthly lending activity and patterns
              </CardDescription>
            </CardHeader>
            <CardContent>
        ...
        <TabsContent value='products' className='space-y-4'>
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Products with highest lending frequency
              </CardDescription>
            </CardHeader>
            <CardContent>
              <ResponsiveContainer width='100%' height={400}>
              ...
              <Card key={product.id}>
                ...
                <CardContent>
                  ...
                  <div className='flex justify-between items-center mb-2'>
                    ...
                    <Badge
                      variant={
                        product.return_rate > 90 ? 'default' : 'destructive'
                      }
                    >
                      {product.return_rate}%
                    </Badge>
                  ...
                  <div className='flex justify-between items-center'>
                    ...
                    <span className='text-sm'>
                      {product.avg_lending_period?.toFixed(1)} days
                    </span>
                  </div>
                </CardContent>
              </Card>
        ...
        <TabsContent value='users' className='space-y-4'>
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Active users and lending activity over time
              </CardDescription>
            </CardHeader>
            <CardContent>
              <ResponsiveContainer width='100%' height={300}>
                ...
                <div className='space-y-2'>
                  <div className='flex justify-between'>
                    ...
                    <Badge variant='destructive'>
                      {overdueAnalytics.overview.total_overdue}
                    </Badge>
                  </div>
                  <div className='flex justify-between'>
                    <span>Avg Days Overdue</span>
                    <span>
                      {overdueAnalytics.overview.avg_days_overdue?.toFixed(1)}
                    </span>
                  </div>
                  <div className='flex justify-between'>
                  ...
                  <div className='flex justify-between'>
                    ...
                    <span>
                      {overdueAnalytics.overview.unique_overdue_borrowers}
                    </span>
                  </div>
                </div>
              </CardContent>
        ...
        <TabsContent value='predictive' className='space-y-4'>
          <div className='grid grid-cols-1 lg:grid-cols-2 gap-4'>
            <Card>
              ...
              <CardContent>
                <div className='space-y-3'>
                  {predictiveAnalytics.trendingProducts
                    ?.slice(0, 5)
                    .map(product => (
                      <div
                        key={product.id}
            ...
            <Card>
              ...
              <CardContent>
                <div className='space-y-3'>
                  {predictiveAnalytics.overdueRiskProducts
                    ?.slice(0, 5)
                    .map(product => (
                      <div
                        key={product.id}
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Optimal times for lending activity
              </CardDescription>
            </CardHeader>
            <CardContent>
              <ResponsiveContainer width='100%' height={300}>
